...
export async function getExchangeRate(): Promise<number> {
    axios.get(process.env.TERRA_API_GET_ENDPOINT)
  .then(function (response) {
    // handle success
    console.log(response);
    log("calling getExchangeRate with response " + response.data.exchange_rate, null);
    return response.data.exchange_rate;
  })
  .catch(function (error) {
    // handle error
    console.log(error);
    log("call to getExchangeRate failed ", null);
    return -1;
  })
  return -1;
}
